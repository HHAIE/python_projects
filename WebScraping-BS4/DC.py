...
url = "https://anime4up.tv/anime/detective-conan/"
...
last_ep64 = [(ep.find_previous_sibling("img").get("alt"), ep.get("href")) for ep in new_eps[0].find_all("a", class_="overlay") if num_from_str(ep.find_previous_sibling("img").get("alt")) not in exist_ep][-64:]
...
for link in down_links:
    ...
    if not os.path.exists("E:/Videos/Videos/Anime/Detective Conan"+"/"+link[1].split("/")[-1]):
        path = "E:/Videos/Videos/Anime/Detective Conan"
        name = "Detective Conan Episode " + str(num_from_str(link[1].split("/")[-1])) + " [Arabic]" + link[1].split("/")[-1][-4:]
